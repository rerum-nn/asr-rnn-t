...
def collate_fn(dataset_items: list[dict]):
    ...
    result_batch["instance_spectrogram"] = [
        item["instance_spectrogram"][0] for item in dataset_items
    ]
    result_batch["instance_audio"] = [item["instance_audio"] for item in dataset_items]

    result_batch["spectrogram_length"] = torch.tensor(
        [item["spectrogram"][0].shape[1] for item in dataset_items], dtype=torch.int32
    )
    result_batch["text_encoded_length"] = torch.tensor(
        [len(item["text_encoded"][0]) for item in dataset_items], dtype=torch.int32
    )
