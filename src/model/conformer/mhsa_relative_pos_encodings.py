...
class MHSAWithRelativePosEncoding(nn.Module):
    def __init__(self, encoder_dim, max_length, attention_heads=4, dropout_rate=0.0):
        ...
        assert encoder_dim % attention_heads == 0, "encoder_dim should be divisible by attention_heads"
        ...
        self.u_vec = nn.Parameter(torch.Tensor(attention_heads, self.head_dim))
        self.v_vec = nn.Parameter(torch.Tensor(attention_heads, self.head_dim))
    ...
    def forward(self, x):
        ...
        value = self.value_mat(x).unflatten(-1, [self.attention_heads, self.head_dim]).transpose(1, 2)
        key = self.key_mat(x).unflatten(-1, [self.attention_heads, self.head_dim]).transpose(1, 2)
        pos_key = self.pos_mat(pos_embedding).unflatten(-1, [self.attention_heads, self.head_dim]).transpose(1, 2)

        content = torch.matmul((query + self.u_vec).transpose(1, 2), key.transpose(2, 3))
        pos = torch.matmul((query + self.v_vec).transpose(1, 2), pos_key.transpose(2, 3))
...
class MultiHeadSelfAttentionModule(nn.Module):
    def __init__(self, encoder_dim, max_length, attention_heads=4, dropout_rate=0.1):
        ...
        self.mhsa = nn.Sequential(
            ...
            MHSAWithRelativePosEncoding(encoder_dim, max_length, attention_heads, dropout_rate=dropout_rate),
